...
def greet_player():
    player_name = input(
        "Welcome to Stone, Parchment, Knife!\n\nWhat should I call you?\n\n> "
    )
    return player_name
...
def get_player_choice():
    while True:
        ...
        if (
            player_choice.lower() == "stone"
            or player_choice.lower() == "parchment"
            or player_choice.lower() == "knife"
        ):
            break
        else:
            print(
                f"{player_choice.title()} is not a valid choice. Please choose again."
            )
